plot_clustered_reads <- function(x, chr, start, end) {
    methy_data <- query_methy(x, chr, start, end) %>%
        filter(.data$pos > start & .data$pos < end) %>%
        inner_join(samples(x), by = "sample")
    ...
    methy_data <- inner_join(methy_data, read_data)
    ...
    p2 <- plot_heatmap_internal(
            methy_data = inner_join(methy_data, cluster_res),
            pos_style = "to_scale",
            subsample = 30,
            group_col = "cluster_id"
        ) +
